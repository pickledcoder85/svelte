...
async def create_log(
    ...
    return create_food_log(repository, _require_session(session), payload)
...
async def read_logs(
    ...
    return list_food_logs(repository, _require_session(session).user_id, week_start, week_end)
...
async def read_log(
    ...
    log = get_food_log(repository, _require_session(session).user_id, food_log_id)
    if log is None:
...
async def read_log_entries(
    ...
    entries = list_food_log_entries(repository, _require_session(session).user_id, food_log_id)
    if entries is None:
        raise HTTPException(status_code=404, detail="Food log not found.")
    return entries
...
async def create_log_entry(
    ...
    log = add_food_log_entry(repository, _require_session(session).user_id, food_log_id, payload)
    if log is None:
        raise HTTPException(status_code=404, detail="Food log not found.")
    return log
